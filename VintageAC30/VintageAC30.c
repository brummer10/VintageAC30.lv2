...
#define PLUGIN_UI_URI "https://github.com/brummer10/VintageAC30_UI"
...
void plugin_set_window_size(int *w,int *h,const char * plugin_uri, float scale) {
    (*w) = 629 * scale; //set initial width of main window
    (*h) = 200 * scale; //set initial height of main window
}

const char* plugin_set_name() {
    return "VintageAC30"; //set plugin name to display on UI
}

void plugin_create_controller_widgets(X11_UI *ui, const char * plugin_uri, float scale) {
    set_costum_theme(&ui->main);
    ui->widget[0] = add_lv2_toggle_button (ui->widget[0], ui->win, 2, "Bypass", ui, 30,  60, 60 * scale, 60 * scale);

    ui->widget[1] = add_lv2_toggle_button (ui->widget[1], ui->win, 3, "TopBoost", ui, 120,  60, 60 * scale, 60 * scale);

    ui->widget[2] = add_lv2_knob (ui->widget[2], ui->win, 4, "Gain1", ui, 210,  45, 60 * scale, 80 * scale);
    set_adjustment(ui->widget[2]->adj, 0.000, 0.000, 0.000, 1.000, 0.010, CL_CONTINUOS);

    ui->widget[3] = add_lv2_knob (ui->widget[3], ui->win, 5, "Gain2", ui, 300,  45, 60 * scale, 80 * scale);
    set_adjustment(ui->widget[3]->adj, 0.000, 0.000, 0.000, 1.000, 0.010, CL_CONTINUOS);

    ui->widget[4] = add_lv2_knob (ui->widget[4], ui->win, 6, "Treble", ui, 360,  45, 60 * scale, 80 * scale);
    set_adjustment(ui->widget[4]->adj, 0.000, 0.000, 0.000, 1.000, 0.010, CL_CONTINUOS);

    ui->widget[5] = add_lv2_knob (ui->widget[5], ui->win, 7, "Bass", ui, 420,  45, 60 * scale, 80 * scale);
    set_adjustment(ui->widget[5]->adj, 0.000, 0.000, 0.000, 1.000, 0.010, CL_CONTINUOS);

    ui->widget[6] = add_lv2_knob (ui->widget[6], ui->win, 8, "Volume", ui, 510,  45, 60 * scale, 80 * scale);
    set_adjustment(ui->widget[6]->adj, 0.000, 0.000, 0.000, 1.000, 0.010, CL_CONTINUOS);
